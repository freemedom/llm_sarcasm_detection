...
from langchain_huggingface import HuggingFaceEndpoint # use huggingface free api
...
class GoCAlgorithm(object):
    def __init__(self, text, llm, ablation_type = None):
        ...
        '''
        # Dictionary: cue_type -> cue text generated by the LLM.
        # Example key: "keywords", value: short extracted cue description.
        self.cues = {}
        # Undirected graph where each node is a cue type and node attribute `text`
        # stores the extracted cue content.
        self.graph = nx.Graph()
        # Ordered list of cue node names, used later for initialization and traversal.
        self.cue_nodes = []
        # Initialization workflow:
        #   Step 1) extract textual content for each cue type
        #   Step 2) materialize those cues as graph nodes and connect them
        self.extract_cues()
        self.construct_graph()

    def extract_cues(self):
        # For each predefined cue type, call the LLM once to extract
        # a concise cue-specific summary from the input text.
        for cue_type in self.cue_types:
            # Build a cue-specific extraction instruction.
            cue_maker_template = f"Extract the brief {cue_type} information from the given text: {self.text}"
            # Wrap instruction as a LangChain prompt.
            cue_maker_prompt = PromptTemplate(template=cue_maker_template, input_variables=["cue_type", "text"])
            # Create a one-step chain and execute it immediately.
            cue_maker_chain = LLMChain(llm=self.llm, prompt=cue_maker_prompt)
            # Persist extracted cue text for downstream graph construction and reasoning.
    ...
    def construct_graph(self):
        ...
        # Add one node per cue type and attach extracted text as node metadata.
        for cue_type, cue_text in self.cues.items():
            self.graph.add_node(cue_type, text=cue_text)
            self.cue_nodes.append(cue_type)
        # Build a fully connected (complete) cue graph:
        # every cue type is linked to every other cue type.
        # This keeps traversal flexible when selecting the next cue.
        ...
        # Debug print to verify node creation and attached attributes.
    ...
    def select_next_cue(self, current_cues):
        ...
        ### The Next Cue:
        """
        # The f-string prefix substitutes {remaining_cues} and {current_cues} when this assignment runs in Python,
        # not later via LangChain PromptTemplate placeholders.
